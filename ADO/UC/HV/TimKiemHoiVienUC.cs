...
namespace ADO.UC.HV
{
    public partial class TimKiemHoiVienUC : UserControl
    {
        ...
        public TimKiemHoiVienUC()
        {
            InitializeComponent();

            list = SinhVienBus.Instance.GetSinhVienHVModels();
        ...
        private void LoadData()
        {
            ...
            else
            {
                if (khoa.ID == 0)
                {
                    list = SinhVienBus.Instance.GetSinhVienHVModels(khoa_hoc.ID);
                }
                else
                {
                    if (nganh.ID == 0)
                    {
                        list = SinhVienBus.Instance.GetSinhVienHVModels(khoa_hoc.ID, khoa.ID);
                    }
                    else
                    {
                        if(lop.ID == 0)
                        {
                            list = SinhVienBus.Instance.GetSinhVienHVModels(khoa_hoc.ID, khoa.ID, nganh.ID);
                        }
                        else
                        {
                            list = SinhVienBus.Instance.GetSinhVienHVModels(khoa_hoc.ID, khoa.ID, nganh.ID, lop.ID);
                        }
                    }
                }
            }
