...
namespace ADO.UC.HV
{
    public partial class HoiVienUC : UserControl
    {
        ...
        public HoiVienUC()
        {
            ...
            list = SinhVienBus.Instance.GetSinhVienHVModelExtent();
        ...
        private void LoadData()
        {
            ...
            if (khoa_hoc.ID == 0)
            {
                list = SinhVienBus.Instance.GetSinhVienHVModelExtent(0, 0, 0, 0, tt_doanPhi);
            }
            else
            {
                if (khoa.ID == 0)
                {
                    list = SinhVienBus.Instance.GetSinhVienHVModelExtent(khoa_hoc.ID, 0, 0, 0, tt_doanPhi);
                }
                else
                {
                    if (nganh.ID == 0)
                    {
                        list = SinhVienBus.Instance.GetSinhVienHVModelExtent(khoa_hoc.ID, khoa.ID, 0, 0, tt_doanPhi);
                    }
                    else
                    {
                        if (lop.ID == 0)
                        {
                            list = SinhVienBus.Instance.GetSinhVienHVModelExtent(khoa_hoc.ID, khoa.ID, nganh.ID, 0, tt_doanPhi);
                        }
                        else
                        {
                            list = SinhVienBus.Instance.GetSinhVienHVModelExtent(khoa_hoc.ID, khoa.ID, nganh.ID, lop.ID, tt_doanPhi);
                        }
                    }
                }
            }
        ...
        private void btnHoiPhi_Click(object sender, EventArgs e)
        {
            if (sv != null)
            {
